...
static void ConditionalUpdateOffroad(Kart::Movement& movement) {
    if (IsBooModeActive() && movement.GetPlayerIdx() < 12 && booActive[movement.GetPlayerIdx()]) return;
    ...
    if (IsBooModeActive() && mgr != nullptr) {
        ...
        for (u8 i = 0; i < count; ++i) {
            ...
            if (kp != nullptr && kp->pointers.kartCollision == &collision && booActive[i]) {
                u32& bf1 = kp->pointers.kartStatus->bitfield1;
                u32 savedBf1 = bf1;
                bf1 |= 0x80000000;
...
void ResetBooStates() {
    ...
    booBehavior.objId = OBJ_MUSHROOM;
